...
<strong style="color: red">重要</strong>：经试验，在同为脱离普通流的绝对定位和固定定位形成的 BFC 是<strong style="color: red">不会</strong>被外层 BFC 包含的。在脱离普通形成的 BFC 中（左或右浮动，绝对和固定定位）<strong style="color: red">只有</strong>浮动元素 BFC 会被外层 BFC 在计算高度时，包含在内。
