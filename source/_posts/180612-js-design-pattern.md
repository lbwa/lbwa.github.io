 ...
 * 设计架构上的一个抽象化概念。如 `水果` 这一抽象概念，`水果` 这一 事物本身 在现实中没有实物的。
...
工厂模式即对构造方法封装的抽象（`abstract`），即对一个功能的抽象集合，即只需要把工厂当作一个 `功能` 即可。它以特定接口实现创建对象的细节的函数封装，微观上，提高代码复用性，避免创建相似对象时产生大量重复代码；宏观上，使得被封装的选择路径功能与应用整体比较时，它被抽象为一个概念，便于架构应用时的整体思考。
...
以上示例代码展示了根据传参不同，而执行不同的实例化路径。将判断路径的这个行为封装为一个工厂方法，集中管理，后续若需要修改某一类时，只需要修改工厂方法即可。若不使用该工厂方法，那么在后期需要删除该类时，需要修改所有引用该类的地方，而使用了工厂方法后，删除该类只需要修改工厂方法即可，即将该类与其他代码解耦。
